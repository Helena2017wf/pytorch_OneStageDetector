...
#     """
#
#     def __init__(self, size ,phase, base, extras, head, num_classes,cgf):
...
#             self.detect = Detect(num_classes,size, 0, 10, 0.01, 0.7) ##for Kaist
#
#     def forward(self, x):
...
class SSD(nn.Module):
    ...
    def __init__(self, size ,phase, num_classes,cfg):
        ...
        base,extras,head = self.multibox(self.vgg(self.config['base'], 3), \
                                         self.add_extras(self.config['extra'], 1024),\
                                         self.config['mbox'], num_classes)
        self._vgg = nn.ModuleList(base)
        # Layer learns to scale the l2 normalized features from conv4_3
        self.L2Norm = L2Norm(512, 20)
        self._extras = nn.ModuleList(extras)

        self._loc = nn.ModuleList(head[0])
        self._conf = nn.ModuleList(head[1])

        if phase == 'test':
            self.softmax = nn.Softmax()
            # self.detect = Detect(num_classes, 0, 200, 0.01, 0.45) ##ssd original
            self.detect = Detect(num_classes,cfg, 0, 10, 0.01, 0.7) ##for Kaist
    ...
    def forward(self, x):
        ...
        # apply vgg up to fc7
        for k in range(23, len(self._vgg)):
            x = self._vgg[k](x)
        sources.append(x)
