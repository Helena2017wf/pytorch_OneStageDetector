...
DatasetRoot = "/data/Sensiac/SensiacFusion"
            ...
            for line in f:
                ...
                if line[0] == "%":
                    ...
                    if not difficult or self.keep_difficult:
                        ...
                        x1, y1, x2, y2 = box
                        xmin = float(x1) / float(width)
                        ymin = float(y1) / float(height)
                        xmax = np.minimum(1.0, float(x2) / float(width))
                        ymax = np.minimum(1.0, float(y2) / float(height))
...
class GetDataset(data.Dataset):
    ...
    def __init__(self, root, transform=None, target_transform=None,
                 type='v_ir_m',dataset_name='day_train10',skip=0):
        self.root = root
        self.type = type
        ...
        self._imgpath = os.path.join('%s', '%s','%s',type,'%s.png')
        self.ids = list()
